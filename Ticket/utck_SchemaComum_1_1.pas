...
// targetNameSpace: http://lote.homologacaoticketfrete.com.br/CDN/fretelote/schemas/SchemaComum_1.1.xsd
// NameSpacePrefix: mstns
// Date of Schema : 25/06/2012 15:24:56
// Translationdate: 25/06/2012 16:38:52
//
...
const
  snPessoaFisica = 'c:PessoaFisica';
  snPessoaFisica_Nome = 'c:Nome';
  snPessoaFisica_Sobrenome = 'c:Sobrenome';
  snPessoaFisica_CPF = 'c:CPF';
  snPessoaFisica_Cartao = 'c:Cartao';
...
const
  thisNamespaceURI = 'http://lote.homologacaoticketfrete.com.br/CDN/fretelote/schemas/SchemaComum_1.1.xsd';
...
class function tPessoaFisica._nsURI_;
begin
  result := thisNameSpaceURI;
end; // class function _nsURI_
...
constructor tPessoaFisica.Create(aRoot:tJanXMLNode2);
var
  xn: tJanXMLNode2;
  sn: string;
//  i: integer;
...
begin
  ...
  begin
    sn := NamePart(xn.name);
    thisURI := xn.namespaceURI;
    if (sn = snPessoaFisica_Nome) then
      Nome :=  xn.text // string
    else if (sn = snPessoaFisica_Sobrenome) then
      Sobrenome :=  xn.text // string
    else if (sn = snPessoaFisica_CPF) then
      CPF :=  xn.text // string
    else if (sn = snPessoaFisica_Cartao) then
...
procedure tPessoaFisica.SetCartao(value:string);
begin
  FCartao := value;
  FCartao_IsSet_ := true;
end;

procedure tPessoaFisica.Save(aNode:tJanXMLNode2);
var
  xn: tJanXMLNode2;
//  i: integer;
begin
  if aNode.name = '' then
    aNode.name := nsName(_NameSpaceAlias, 'PessoaFisica')
  else if pos(colon, aNode.name) = 0 then
    aNode.name := nsName(_NameSpaceAlias, aNode.name);
  ...
  xn := aNode.addChildByName(nsName(_NameSpaceAlias, snPessoaFisica_Nome));
  xn.text := FNome;
  xn := aNode.addChildByName(nsName(_NameSpaceAlias, snPessoaFisica_Sobrenome));
  xn.text := FSobrenome;
  xn := aNode.addChildByName(nsName(_NameSpaceAlias, snPessoaFisica_CPF));
  xn.text := FCPF;
  if Cartao_IsSet_ then
  begin
    xn := aNode.addChildByName(nsName(_NameSpaceAlias, snPessoaFisica_Cartao));
    xn.text := FCartao;
  end;
end; // procedure save
