...
const
  ...
  dutchmonthnames: array[1..12] of string = ('januari', 'februari', 'maart',
    'april', 'mei', 'juni', 'juli', 'augustus', 'september', 'oktober',
    'november',
    'december');
  dutchdaynames: array[1..7] of string = ('zondag', 'maandag', 'dinsdag',
...
procedure XMLSetAttribute(var asource: string; aname, avalue: string; aposition:
  integer);
...
function match2(strSource, strFirst, strSecond: string; startPos: integer; var
  p1: integer; var p2: integer): boolean;
// match strFirst*strSecond*strThird
function match3(strSource, strFirst, strSecond, strThird: string; startPos:
  integer; var p1: integer; var p2: integer; var p3: integer): boolean;
// scans for next occurance of strScan
function scannext(strSource, strScan: string; startPos: integer; var scanPos:
  integer): boolean;
...
function hithighlight(fn: string; searchwords: tstringlist; myDir: string;
  abackcolor, aforecolor: string; var hits: integer): string;
function strHithighlight(aText: string; searchwords: tstringlist; abackcolor,
  aforecolor: string; var hilites: integer): string;
...
function GetFirstDayofMonthCalendar(const DT: TDateTime; const pWeekDay:
  integer): TDateTime;
...
function InsertIndexBlock(var sourceStr: string; blockStr: string; index:
  integer): boolean;
...
function EncryptB64(const InString: string; StartKey, MultKey, AddKey: Integer):
  string;
function DecryptB64(const InString: string; StartKey, MultKey, AddKey: Integer):
  string;
...
function PosstrBefore(const FindString, SourceString: string; startPos:
  integer): integer;
function PosStrLast(const FindString, SourceString: string): integer;
{finds the last occurance}

function StrRScan(const S: string; Ch: Char; LastPos: Integer = MaxInt):
  Integer;
...
function PosTextBefore(const FindString, SourceString: string; startPos:
  integer): integer;
...
function Q_ReplaceStr(const SourceString, FindString, ReplaceString: string):
  string;
function Q_ReplaceText(const SourceString, FindString, ReplaceString: string):
  string;
...
function findClosingTag(source, tagname: string; startpos: integer; var
  foundpos: integer; casesensitive: boolean = false): boolean;
function findOpeningTag(source, tagname: string; startpos: integer; var
  foundpos: integer; casesensitive: boolean = false): boolean;
...
function PosEscaped(Start: integer; SourceText, FindText: string; escapeChar:
  char): integer;
...
function ParseAttribute(var Start: integer; SourceText: string; var aName:
  string; var aValue: string): boolean;
...
function PosRangeStr(Start: integer; HeadString, TailString, SourceString:
  string; var RangeBegin: integer; var RangeEnd: integer): boolean;
// finds a text range, e.g. <TD>....</TD> case sensitive
function PosRangeText(Start: integer; HeadString, TailString, SourceString:
  string; var RangeBegin: integer; var RangeEnd: integer): boolean;
// finds a text range, e.g. <TD>....</td> case insensitive
function BackPosRangeStr(Start: integer; HeadString, TailString, SourceString:
  string; var RangeBegin: integer; var RangeEnd: integer): boolean;
// finds a text range backward, e.g. <TD>....</TD> case sensitive
function BackPosRangeText(Start: integer; HeadString, TailString, SourceString:
  string; var RangeBegin: integer; var RangeEnd: integer): boolean;
...
function Innertag(Start: integer; HeadString, TailString, SourceString: string;
...
begin
  ...
  begin
    Result := (Result shr 4) xor (((Result xor janHashTable[aString[I]]) and $F)
      * $1000);
    Result := (Result shr 4) xor (((Result xor (ord(janHashTable[aString[I]])
      shr
      4)) and $F) * $1000);
  end;
  if Result = 0 then
...
begin
  if not fileexists(aFile) then
    SaveString(aFile, '');
  with TFileStream.Create(aFile, fmOpenReadWrite) do
  try
    Seek(0, soFromEnd);
    writeBuffer(aText[1], length(aText));
  finally free;
...
var
  ...
  begin
    ...
    if p1 = 1 then
      exit;
    if asource[pp - 1] = ';' then
      exit;
    while (pp > 1) and (asource[pp - 1] = ' ') do
      dec(pp);
    if pp = 1 then
      exit;
    if asource[pp - 1] = ';' then
...
begin
  ...
  if RangeBegin = 0 then
    exit;
  RangeEnd := PosText(TailString, SourceString, RangeBegin +
    Length(HeadString));
  if RangeEnd = 0 then
    exit;
  RangeEnd := RangeEnd + length(TailString) - 1;
...
begin
  ...
  if Start < 1 then
    exit;
  repeat
    if not PosRangeStr(Start, HeadString, TailString, SourceString, RangeBegin,
      RangeEnd) then
      exit;
...
begin
  ...
  if Start < 1 then
    exit;
  repeat
    if not PosRangeText(Start, HeadString, TailString, SourceString, RangeBegin,
      RangeEnd) then
      exit;
    if RangeBegin < Start then
    begin
...
begin
  ...
  c := length(avalue);
  i := 1;
  while (i <= c) and (avalue[i] in ['0'..'9']) do
    inc(i);
  if i = 1 then
    exit;
...
function PosStr(const FindString, SourceString: string; StartPos: Integer):
  Integer;
asm
        PUSH    ESI
        PUSH    EDI
...
function PosText(const FindString, SourceString: string; StartPos: Integer):
  Integer;
asm
        PUSH    ESI
        PUSH    EDI
...
begin
  ...
  p := posstr('*', FindString);
  if p = 0 then
    exit; // must have wild card
  if (p = 1) or (p = length(FindString)) then
    exit; // * may not be first or last character
...
begin
  ...
  begin
    ...
    OutBuf[1] := B64Table[(((InBuf[0] and $03) shl 4) or ((InBuf[1] and $F0) shr
      4)) + 1];
    OutBuf[2] := B64Table[(((InBuf[1] and $0F) shl 2) or ((InBuf[2] and $C0) shr
      6)) + 1];
...
begin
  ...
  if length(set2) > length(set1) then
  begin
    s1 := set1;
    s3 := ',' + set2 + ',';
  end
...
function setCheckgroup(var source: string; name, value: string): boolean;
...
begin
  ...
  begin
    ...
    if (bov < eot) and (sTag = 'input') and (bok < eot) then
    begin
      // found it
 ...
 { delim := '''';}
  ...
  begin
    ...
    begin
      ...
      // find eov
      eov := bov + 1;
      while (eov < eot) and (source[eov] <> delim) do
        inc(eov);
      if eov < eot then
      begin
        // found it
...
begin
  ...
  if p = 0 then
    exit;
  result := copy(sourcestr, 1, p - 1) + replacestr + copy(sourceStr, p +
    length(findStr), length(sourceStr));
end;
...
begin
  ...
  SourceStr := copy(SourceStr, 1, pblock - 1) + sbr + copy(SourceStr, pblock,
    length(sourceStr));
  result := true;
end;
  ...
  repeat
    ...
    begin
      ...
      begin
        ...
        begin // close tag
          if pb > (peold + 1) then
            s := s + stringofChar(' ', level * 2) + copy(aText, peold + 1, pb -
              peold - 1) + cr;
          if level > 1 then
            dec(level);
          s := s + stringofChar(' ', level * 2) + copy(aText, pb, pe - pb + 1) +
            cr;
        end
        else
        begin
...
begin
  ...
  begin
    ...
    if ch = '>' then
      isTag := false;
    if (ch in charset) and (not istag) then
    begin
      if not haveword then
      begin
        word := '';
...
begin
  ...
  begin
    ...
    repeat
      ...
      if pb > 0 then
      begin // check of not in tag
        ptb := posstr('<', page, pb);
        pte := posstr('>', page, pb);
        if ptb < pte then
        begin // not in tag
          insert('</b>', page, pb + length(sword));
          insert('<a name="bshh' + inttostr(hilites) + '"></a>' + strHL, page,
            pb);
          inc(hilites);
  ...
  repeat
    ...
    begin
      ...
      if (postext('http://', href) = 0) and (copy(href, pb + 6, 1) <> '#') then
      begin //must adjust
...
function strHithighlight(aText: string; searchwords: tstringlist; abackcolor,
  aforecolor: string; var hilites: integer): string;
var
  sword, page, {href, hedir, }si: string;
...
begin
  ...
  repeat
    ...
    begin
      ...
      if pde > 0 then
      begin
        result := copy(result, 1, pdb + length(zip) - 1) + part + copy(result,
          pde, maxint);
      end;
    end;
  until pe = 0;
...
begin
  result := 0;
  try
    result := encodedate(strtoint(copy(atext, 1, 4)), strtoint(copy(atext, 6,
      2)), strtoint(copy(atext, 9, 2)));
  except
  end;
end;
...
begin
  ...
  case akind of
    2: result := dutchdaynames[dayofweek(now)] + ' ' + inttostr(day) + ' ' +
      dutchmonthnames[month] + ' ' + inttostr(year);
    1: result := inttostr(day) + ' ' + dutchmonthnames[month] + ' ' +
      inttostr(year);
  else
...
begin
  ...
  if copy(asource, aposition, 1) <> '<' then
    exit;
  if axml = '' then
    exit;
...
begin
  result := '';
  L := length(asource);
  if (aposition < 0) or (aposition > L) then
    exit;
  if asource[aposition] <> '<' then
    exit;
  p2 := posstr('>', asource, aposition);
  if p2 = 0 then
    exit;
  p1 := aposition + 1;
  while (p1 <= L) and (not (asource[p1] in [' ', '>'])) do
    inc(p1);
  if p1 > L then
    exit;
...
begin
  ...
  if not (delim in ['''', '"']) then
    exit;
  inc(p2);
  while (p2 <= L) and (asource[p2] <> delim) do
    inc(p2);
  if p2 > L then
    exit;
...
begin
  ...
  if p1 = 0 then
    exit;
  // check for /> shortcut
  if copy(asource, p1 - 1, 1) = '/' then
    exit;
  p2 := PosStr('<', asource, p1);
  if p2 = 0 then
    exit;
  if copy(asource, p2, 9) = '<![CDATA[' then
  begin
    // CDATA section
    p1 := p2 + 9;
    p2 := PosStr(']]>', asource, p1);
...
const
  ...
  = (0, 1, 2, 3, 0, 1, 2, 0, 0, 2, 2, 4, 5, 5, 0, 1, 2, 6, 2, 3, 0, 1, 0, 2, 0,
    2, 0, 0, 0, 0, 0, 0,
...
begin
  ...
  raise exception.Create('Procedimetno não suportado');
end;
...
//  p1f := p1;
  ...
  begin
    ...
    if copy(src, p - 2, 2) = '</' then
    begin // closing tag
      p1f := p;
      if copy(src, p + length(atag), 1) = '>' then
      begin
        dec(hits);
        if hits = 0 then
        begin
