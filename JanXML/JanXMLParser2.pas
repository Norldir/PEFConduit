...
uses
  ...
  janXPathTokenizer, Math, janstrings;
...
type
  ...
  TVariableEvent = procedure(sender: Tobject; const VariableName: string; var
    VariableValue: variant; var handled: boolean) of object;
  TXELEvent = procedure(sender: TObject; node: TjanXMLNode2; const Act: string)
    of object;
  TjanXSLSort = record
    pattern: string;
  ...
  public
    ...
    procedure selectNodes(node: TjanXMLNode2; nodelist: TList; single: boolean =
      false);
  ...
  public
    ...
    property attribute[index: variant]: string read getAttribute write
    ...
    property name1 : string read GetName1;
    {le sem o namespace}
    ...
    procedure selectNodes(nodelist: TList; pattern: string; single: boolean =
      false); overload;
    {Adds recursive child nodes that match pattern to nodelist.}
    procedure selectNodes(parser: TjanXMLParser2; nodelist: TList; pattern:
      string; single: boolean = false); overload;
    {Adds recursive child nodes that match pattern to nodelist.
    ...
    function getChildByName(aname: string; descent: boolean = false):
      TjanXMLNode2;
    ...
    function getChildByAttribute(AttributeName, AttributeValue: string):
      TjanXMLNode2;
  ...
  private
    ...
    FpageSize: integer;
    FonXEL: TXELEvent;
    FformatedXML: boolean;
    FXMLTag: boolean;
    function getXML: string;
    procedure setXML(const Value: string);
    function AsText: string;
    procedure OutputNode(node: TjanXMLNode2);
    procedure XSLTOutputNode(node, context: TjanXMLNode2; matchlist,
      templatelist: TList);
    procedure ExecXSLT(node, context: TjanXMLNode2; matchlist, templatelist:
      TList);
    procedure xsl_value_of(node, context: TjanXMLNode2; matchlist, templatelist:
      TList);
    procedure xsl_call_template(node, context: TjanXMLNode2; matchlist,
      templatelist: TList);
    procedure xsl_apply_templates(node, context: TjanXMLNode2; matchlist,
      templatelist: TList);
    procedure xsl_if(node, context: TjanXMLNode2; matchlist, templatelist:
      TList);
    procedure xsl_attribute(node, context: TjanXMLNode2; matchlist,
      templatelist: TList);
    procedure xsl_element(node, context: TjanXMLNode2; matchlist, templatelist:
      TList);
    procedure xsl_comment(node, context: TjanXMLNode2; matchlist, templatelist:
      TList);
    procedure xsl_choose(node, context: TjanXMLNode2; matchlist, templatelist:
      TList);
    procedure xsl_for_each(node, context: TjanXMLNode2; matchlist, templatelist:
      TList);
    ...
    procedure Sort(matchlist: TList; From, Count: Integer; orderby: array of
      TjanXSLSort);
    function Compare(matchlist: TList; i, j: Integer; orderby: array of
      TjanXSLSort): Integer;
  ...
  public
    ...
    property NodeXML[pNode: TjanXMLNode2]: string read getNodeXML write
    setNodeXML;
    ...
    property formatedXML: boolean read FformatedXML write FformatedXML;
    property XMLTag: boolean read FXMLTag write FXMLTag;
  published
  ...
  private
    ...
    procedure GetAttribute(sender: Tobject; const VariableName: string; var
      VariableValue: variant; var handled: boolean);
  ...
  public
    ...
    property onGetAttribute: TVariableEvent read FonGetVariable write
      SetonGetVariable; //event
...
procedure TjanXMLNode2.selectNodes(nodelist: TList; pattern: string; single:
  boolean = false);
var
  {i, c: integer;}
...
begin
  ...
  if c = 0 then
    exit;
  for i := 0 to c - 1 do
    if TjanXMLAttribute2(FAttributes[i]).name = aname then
    begin
      result := i;
      exit;
    end;
...
function TjanXMLNode2.getChildByID(aid: string; descent: boolean = false):
  TjanXMLNode2;
var
  i, c: integer;
  n: TjanXMLNode2;
...
function TjanXMLNode2.getChildByName(aname: string; descent: boolean = false):
  TjanXMLNode2;
var
  i, c: integer;
  n: TjanXMLNode2;
...
var
  ...
  begin
    ...
    if result = nil then
      exit;
    if result.name <> 'xel:function' then
      result := nil;
  end;
begin
  result := false;
  atom := name;
  if pos('xel:', atom) = 1 then
  begin
    act := self['act'];
    xpp := parser.getXPathParser(act);
    delete(atom, 1, 4);
    if atom = 'program' then
      skip := true
    else if atom = 'function' then
      skip := true
    else if atom = 'var' then
      skip := true
    else if atom = 'do' then
    begin
      xpp.testNode(self);
      exit;
    end
    else if atom = 'call' then
    begin
    ...
    else if atom = 'if' then
    begin
      if not xpp.testNode(self) then
        exit;
      c := nodes.count;
      if c = 0 then
        exit;
    ...
    else if atom = 'while' then
    begin
      t1 := gettickcount;
      while xpp.testNode(self) do
      begin
        t2 := gettickcount;
        if (t2 - t1) > 10000 then
          exit;
    ...
    else if atom = 'switch' then
    begin
      result := xelSwitch(self, parser);
      exit;
    end
    else if atom = 'custom' then
    begin
...
begin
  ...
  begin
    ...
    if xcase.name <> 'xel:case' then
      continue;
    xpp := parser.getXPathParser(xcase['act']);
    if not xpp.testNode(xcase) then
      continue;
...
function TjanXMLNode2.GetName1: string;
begin
   result := StringReplace( name, namespace+':', '',[]);
end;
...
begin
  try
     ...
     fn := self; //.FirstChild;
     //ShowMessage( intToStr(self.count)+'  '+intToStr( fn.nodes.Count  ));
     OutputNode(fn );
...
begin
  ...
  // <![CDATA[   ... ]]>
  result := value;
  if (posstr('<', value) = 0) and (posstr('>', value) = 0) then
    exit;
  result := '<![CDATA[' + value + ']]>';
end;
  ...
  try
    ...
    begin
      if Fscan < 21 then
        ...
        p := Fscan - 20;
      src := copy(Fxml, p, 50);
      result := e.Message + ' near ' + src + cr + 'Error position=' +
        inttostr(Fscan); // TK:2005-07-07 removed + cr + AsText;
    end;
  end
end;
...
begin
  ...
  begin
    ...
    if p2 = 0 then
      raise exception.Create('Expected closing ' + delim +
...
var
  p, p1, p2: integer;
  tagname, strAttributes: string;
  bShortcut: boolean;
  tag: string;
  upnode: tJanXMLNode2;
  newNode: tJanXMLNode2;
  currentnode: tJanXMLNode2;
...
begin
  ...
  repeat
    ...
    begin
      // closing tag
      FScan := p + 1;
      currentnode := currentnode.FParentNode;
    end
    else
    begin
      fscan := p + 1;
...
begin
  ...
  if p1 = 0 then
    raise exception.Create('Expected < when parsing text');
  // check for <![CDATA[
  if copy(Fxml, p1, 9) = '<![CDATA[' then
  begin
    Fscan := p1 + 9;
    p1 := posstr(']]>', Fxml, fscan);
    if p1 = 0 then
      raise exception.Create('Expected ]]> when parsing CDATA section');
...
begin
  ...
  if L = 0 then
    exit;
  tmp := value;
  if (FXMLPosition + L) > FXMLSize then
  begin
    if L > FpageSize then
      FXMLSize := FXMLSize + L
    else
...
begin
  ...
  FOutputdepth := 0;
  result := '';
  if self.name <> 'xsl:stylesheet' then
    exit;
  try
...
procedure TjanXMLParser2.XSLTOutputNode(node, context: TjanXMLNode2; matchlist,
  templatelist: TList);
var
  i, c: integer;
  att, attname, attvalue: string;
...
begin
  ...
  begin
    ...
    if nodename[1] = '{' then
    begin
      if nodename[length(nodename)] <> '}' then
        exit;
  ...
  else if node.namespace = 'xsl' then
  begin
...
procedure TjanXMLParser2.xsl_call_template(node, context: TjanXMLNode2;
  matchlist,
  templatelist: TList);
var
  templatename: string;
  ...
  begin
    ...
    begin
      ...
      if nsort.name <> 'xsl:sort' then
        continue;
      inc(result);
      setlength(orderby, result);
      orderby[result - 1].pattern := nsort.attribute['select'];
      orderby[result - 1].SortAscending := nsort.attribute['order'] <>
        'descending';
      orderby[result - 1].SortNumeric := nsort.attribute['data-type'] =
        'number';
    end;
  end;
begin
  ...
  begin
    ...
    if templateNode.attribute['name'] <> templatename then
      continue;
    match := templateNode.attribute['match'];
    ct := templatenode.nodes.count;
    if ct = 0 then
    ...
    try
      ...
      begin
        ...
        begin
          ...
          for it := 0 to ct - 1 do
          begin
            XSLTOutputNode(TjanXMLNode2(templatenode.nodes[it]),
              TjanXMLNode2(matchlist_[ii]), matchlist_, templatelist);
          end;
...
begin
  ...
  if xpp.XPath.Evaluate then
    for i := 0 to c - 1 do
    begin
      XSLTOutputNode(TjanXMLNode2(node.Fnodes.items[i]), context, matchlist,
        templatelist);
    end;
end;
...
begin
  ...
  begin
    n := TjanXMLNode2(node.nodes[i]);
    if n.name = 'xsl:otherwise' then
    begin
      cw := n.nodes.count;
      if cw = 0 then
        exit;
      for iw := 0 to cw - 1 do
        XSLTOutputNode(TjanXMLNode2(n.nodes.items[iw]), context, matchlist,
          templatelist);
      exit;
    end;
    if n.name <> 'xsl:when' then
      continue;
  ...
  try
    ...
    if cc > 0 then
      ...
      begin
        ...
        for i := 0 to c - 1 do
        begin
          XSLTOutputNode(TjanXMLNode2(node.nodes[i]),
            TjanXMLNode2(matchlist_[ii]), matchlist_, templatelist);
        end;
        XMLPut(cr);
      end;
...
procedure TjanXMLParser2.Sort(matchlist: TList; From, Count: Integer;
  ...
  begin
    repeat
      ...
      repeat
        while Compare(matchlist, From + L, From + M, orderby) < 0 do
          Inc(L);
        while Compare(matchlist, From + M, From + R, orderby) < 0 do
          Dec(R);
        if L <= R then
        begin
...
begin
  ...
  begin
    ...
    if v = null then
      ...
      s2 := v;
    if orderby[obi].SortAscending then
    begin
...
begin
  ...
  for i := 0 to c - 1 do
  begin
    XSLTOutputNode(TjanXMLNode2(node.nodes[i]), context, matchlist,
      templatelist);
  end;
  XMLPut('</' + tmp + '>');
end;
...
begin
  ...
  if encoding = '' then
    encoding := 'ISO-8859-1';
  if pNode.parentNode = nil then
    XMLPut('<?xml version=''' + version + ''' encoding=''' + encoding + ''' ?>'
      + cr);
  OutputNode(pNode);
  result := copy(FOutput, 1, FXMLPosition);
  FOutput := '';
  ...
  try
    ...
    begin
      ...
      begin
        ...
        matchlist_.Clear;
        context.selectNodes(self, matchlist_, match);
        if matchlist_.IndexOf(SelNode) <> -1 then
        begin
          for it := 0 to ct - 1 do
          begin
            XSLTOutputNode(TjanXMLNode2(templatenode.nodes[it]), SelNode,
              selectlist_, templatelist);
          end;
          XMLPut(cr);
...
procedure TjanXPathParser2.selectNodes(node: TjanXMLNode2; nodelist: TList;
...
begin
  ...
  begin
    ...
    case tok.tokenkind of
      tkOperand: if not InFixToPostFix(i) then
          exit;
      tkOpen: if not InFixToStack(i) then
          exit;
      tkClose: if not CloseStackToPostFix then
          exit;
      tkOperator:
        begin
          if not OperatorsToPostFix(tok.level) then
            exit;
          InFixToStack(i);
        end;
    end;
...
begin
  result := false;
  while (FStack.count <> 0) and (TToken(Fstack[FStack.count - 1]).tokenkind <>
    tkOpen) do
    StackToPostFix;
...
begin
  while (FStack.count <> 0) and (TToken(Fstack[FStack.count - 1]).level >= level)
    do
    StackToPostFix;
  result := true;
end;
...
begin
  result := false;
  if (index < 0) or (index >= FInFix.count) then
    exit;
...
begin
  ...
  else if (copy(s1, 1, 1) = '%') and (copy(s1, length(s1), 1) = '%') then
  begin
    s1 := copy(s1, 2, length(s1) - 2);
    result := postext(s1, s2) > 0;
  end
  else if (copy(s1, 1, 1) = '%') then
  begin
    s1 := copy(s1, 2, maxint);
    p1 := postext(s1, s2);
    result := p1 = length(s2) - length(s1) + 1;
  end
  else if (copy(s1, length(s1), 1) = '%') then
  begin
...
begin
  ...
  if assigned(onGetAttribute) then
  begin
    handled := false;
...
begin
  ...
  if CurrentNode.hasAttribute(variablename) then
  begin
...
begin
  ...
  begin
    ...
    for i := 0 to c - 1 do
      if TjanXMLNode2(CurrentNode.nodes[i]).name = s then
      begin
        runpush(true);
        exit;
      end;
