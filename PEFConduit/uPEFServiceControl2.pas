...
function ServiceGetList( sMachine : string; dwServiceType, dwServiceState : DWord;
         slServicesList : TStrings; NomeBase : String ): boolean;
procedure GetServiceInfo(AService: string; out StartType: DWord; out Path: string);

implementation

uses WinSvc,SysUtils, dialogs;


function DLLGetLastError : Integer;
begin
 result := GetLastError;
end;
...
begin
  ...
  repeat
    if ControlService( ServiceHandle, SERVICE_CONTROL_INTERROGATE, ServiceStatus)  then
    begin
      Error := DLLGetLastError;
  ...
  try
    ...
    if SCMHandle = 0 then
      RaiseException('OpenSCManager@SimpleStopService: ' + AService + ' ' +
        SysErrorMessage(DLLGetLastError));
    try
      ServiceHandle := OpenService(SCMHandle, PChar(AService), SERVICE_ALL_ACCESS);
      if ServiceHandle = 0 then
    ...
    try
      ...
      if ServiceHandle = 0 then
        RaiseException('OpenService@SimpleStartService: ' + AService + ' ' +
          SysErrorMessage(DLLGetLastError));
      try
        if StartService(ServiceHandle, 0, argument) then
        begin
          Error := DLLGetLastError;
  ...
  try
    ...
    try
      ...
      if ServiceHandle = 0 then
        RaiseException('SimpleSetServiceStartup@OpenService: ' + AService + ' ' +
          SysErrorMessage(DLLGetLastError));
      if QueryServiceConfig(SCMHandle, nil, 0, bytes_needed) then
        Assert(False, 'QueryServiceConfig should have failed.');
      if GetLastError <> Error_Insufficient_Buffer then
        RaiseLastOSError;

      GetMem(config, bytes_needed);
      try
        Win32Check(QueryServiceConfig(SCMHandle, config, bytes_needed, bytes_needed));
...
begin
  ...
  begin
    ...
    begin
      nomeServico := StrPas( ssa^[j].lpServiceName );
      if Pos( NomeBase, nomeServico ) > 0 then
        slServicesList.Add( nomeServico );
    end;
