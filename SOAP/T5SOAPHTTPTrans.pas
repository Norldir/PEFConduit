...
type
  ...
  TT5HTTPReqResp = class;

  { Provides access to HTTPReqResp component }
  IHTTPReqResp = interface
  ['{5FA6A197-32DE-4225-BC85-216CB80D1561}']
    function GeTT5HTTPReqResp: TT5HTTPReqResp;
  end;

  TBeforePostEvent = procedure(const HTTPReqResp: TT5HTTPReqResp; Data: Pointer) of object;
  ...
  TT5HTTPReqResp = class(TComponent, IInterface, IWebNode, IHTTPReqResp)
  private
    FUserSetURL: Boolean;
    FRefCount: Integer;
...
constructor TT5HTTPReqResp.Create(Owner: TComponent);
begin
  inherited;
{$IFNDEF USE_INDY}
  FInetRoot := nil;
...
destructor TT5HTTPReqResp.Destroy;
begin
{$IFNDEF USE_INDY}
...
class function TT5HTTPReqResp.NewInstance: TObject;
begin
  Result := inherited NewInstance;
  TT5HTTPReqResp(Result).FRefCount := 1;
end;

procedure TT5HTTPReqResp.AfterConstruction;
begin
  inherited;
...
function TT5HTTPReqResp._AddRef: Integer;
begin
  Result := InterlockedIncrement(FRefCount)
end;

function TT5HTTPReqResp._Release: Integer;
begin
...
{$IFNDEF USE_INDY}
procedure TT5HTTPReqResp.Check(Error: Boolean; ShowSOAPAction: Boolean);
var
  ErrCode: Integer;
  S: string;
begin
...
function TT5HTTPReqResp.GeTT5HTTPReqResp: TT5HTTPReqResp;
begin
  Result := Self;
end;

function TT5HTTPReqResp.GetSOAPAction: string;
begin
...
procedure TT5HTTPReqResp.SetSOAPAction(const SOAPAction: string);
begin
  FSoapAction := SOAPAction;
end;

procedure TT5HTTPReqResp.SetWSDLView(const WSDLVIew: TWSDLView);
begin
  FWSDLView := WSDLView;
end;

procedure TT5HTTPReqResp.SetURL(const Value: string);
begin
  if Value <> '' then
    FUserSetURL := True
  else
...
procedure TT5HTTPReqResp.InitURL(const Value: string);
{$IFNDEF USE_INDY}
var
...
procedure TT5HTTPReqResp.SetMimeBoundary(Value: string);
begin
  FMimeBoundary := Value;
end;

function TT5HTTPReqResp.GetMimeBoundary: string;
begin
  Result := FMimeBoundary;
end;

procedure TT5HTTPReqResp.SetUsername(const NameValue: string);
begin
  FUserName := NameValue;
...
procedure TT5HTTPReqResp.SetPassword(const PasswordValue: string);
begin
...
procedure TT5HTTPReqResp.SetProxy(const ProxyValue: string);
begin
  FProxy := ProxyValue;
  if Assigned(WSDLView) then
...
function TT5HTTPReqResp.GetSOAPActionHeader: string;
begin
  if (SoapAction = '') then
...
procedure TT5HTTPReqResp.Connect(Value: Boolean);
var
  AccessType: Integer;
begin
  if Value then
...
procedure  TT5HTTPReqResp.Receive(Context: Integer; Resp: TStream; IsGet: Boolean);
var
...
function TT5HTTPReqResp.Send(const ASrc: TStream): Integer;
var
  Request: HINTERNET;
  RetVal, Flags: DWord;
  P: Pointer;
...
begin
  ...
  try
    ...
    if FConnectTimeout > 0 then
      {Check veja:http://www.delphigroups.info/2/10/344289.html}(InternetSetOption(Request, INTERNET_OPTION_CONNECT_TIMEOUT, Pointer(@FConnectTimeout), SizeOf(FConnectTimeout)));
    if FSendTimeout > 0 then
      {Check veja:http://www.delphigroups.info/2/10/344289.html}(InternetSetOption(Request, INTERNET_OPTION_SEND_TIMEOUT, Pointer(@FSendTimeout), SizeOf(FSendTimeout)));
    if FReceiveTimeout > 0 then
      {Check veja:http://www.delphigroups.info/2/10/344289.html}(InternetSetOption(Request, INTERNET_OPTION_RECEIVE_TIMEOUT, Pointer(@FReceiveTimeout), SizeOf(FReceiveTimeout)));
    ...
    begin
      ...
      if self.ContentType <> '' then
        ContentHeader := Format(ContentTypeTemplate, [self.ContentType])
      else if UseUTF8InHeader then
...
function TT5HTTPReqResp.SendGet: Integer;
var
  Request: HINTERNET;
...
{$IFDEF USE_INDY}
procedure TT5HTTPReqResp.SetupIndy(IndyHttp: TIDHttp; Request: TStream);
...
begin
  ...
  // IndyHttp.HTTPOptions := [hoKeepOrigProtocol];
...
{  if Request is TMimeAttachmentHandler then }
  ...
  //IndyHttp.Request.URL :=  FUrlHost;
  //IndyHttp.BoundPort := FUrlPort;
end;
{$ENDIF}

procedure TT5HTTPReqResp.Get(Resp: TStream);
{$IFNDEF USE_INDY}
var
  Context: Integer;
{$ENDIF}
...
procedure TT5HTTPReqResp.BeforeExecute(const IntfMD: TIntfMetaData;
                                     const MethMD: TIntfMethEntry;
                                     MethodIndex: Integer;
                                     AttachHandler: IMimeAttachmentHandler);
var
...
procedure TT5HTTPReqResp.Execute(const DataMsg: String; Resp: TStream);
var
...
function TT5HTTPReqResp.Execute(const Request: TStream): TStream;
begin
  Result := TMemoryStream.Create;
  Execute(Request, Result);
end;

procedure TT5HTTPReqResp.CheckContentType;
begin
...
procedure TT5HTTPReqResp.Execute(const Request: TStream; Response: TStream);
...
procedure TT5HTTPReqResp.Debug(msg: string);
var
   arq : textfile;
   f : string;
begin
   ...
   begin
       f := ExtractFilePath( application.exename )+'TT5HTTPReqResp.txt';
       AssignFile( arq, f );
       try
         if FileExists( f ) then Append(arq) else Rewrite( arq );
         Writeln( arq, msg );
       finally
         closefile( arq );
       end;
