unit urdc_jxml_TiposPadrao_v1_01;
...
// NameSpacePrefix: rdc
// Date of Schema : 18/07/2012 16:54:00
// Translationdate: 28/03/2013 16:22:19
//
...
begin
  ...
  begin
    ...
    if (sn = snRegistroOperacaoRetorno_IDOperacaoViagem) then
    ...
    else if (sn = snRegistroOperacaoRetorno_StatusRetorno) then
      FStatusRetorno := xn.text
...
begin
  ...
  begin
    sn := NamePart(xn.name);
    thisURI := xn.namespaceURI;
    if (sn = snRegistroOperacaoViagem_IDOperacaoViagem) then
      FIDOperacaoViagem := xn.text
      ...
      and ((thisURI='') or (tEventoOperacao._nsURI_ = thisURI)) then
      FEventoOperacao := tEventoOperacao.Create(xn)
    else if (sn = snRegistroOperacaoViagem_NumeroCartao) then
      FNumeroCartao := xn.text
    else if (sn = snRegistroOperacaoViagem_CPFMotorista)then
      FCPFMotorista := xn.text
    ...
    else if (sn = snRegistroOperacaoViagem_PesoCarga)then
      FPesoCarga := DefStrToFloat(xn.text,0)
    else if (sn = snRegistroOperacaoViagem_UnidadeMedidaPeso) then
...
begin
  ...
  begin
    ...
    if (sn = snEventoOperacaoRetorno_IDEventoOperacaoViagem) then
      ...
      ValorEvento := DefStrToFloat( xn.text,0) // tValorType
    ...
    else if (sn = snEventoOperacaoRetorno_ValorTaxaEventoOperacaoViagem) then
      FValorTaxaEventoOperacaoViagem := DefStrToFloat(xn.text, 0)
    else if (sn = snEventoOperacaoRetorno_SituacaoEventoOperacaoViagem) then
      FSituacaoEventoOperacaoViagem := xn.text
...
begin
  ...
  xn.text := MyFloatToStr( FValorTaxaEventoOperacaoViagem );

  xn := aNode.addChildByName(snEventoOperacaoRetorno_SituacaoEventoOperacaoViagem);
  xn.text := FSituacaoEventoOperacaoViagem ;
...
begin
  ...
  begin
    sn := NamePart(xn.name);
    thisURI := xn.namespaceURI;
    if (sn = snOperacaoViagemRetorno_IDOperacaoViagem) then
      FIDOperacaoViagem := xn.text
    ...
    else if (sn = snOperacaoViagemRetorno_SituacaoOperacaoViagem) then
      FSituacaoOperacaoViagem := xn.text
    else if (sn = snOperacaoViagemRetorno_StatusRetorno) then
      FStatusRetorno := xn.text
